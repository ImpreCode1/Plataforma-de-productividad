...
def get_areas(db: DBSession, current_user: CurrentUser):
...
@router.patch("/{user_id}/status", response_model=UserResponse, dependencies=[Depends(require_roles("ADMIN"))])
def change_status(user_id: UUID, data: ChangeStatusRequest, db: DBSession, current_user: CurrentUser):
    return service.change_status(db, user_id, data.is_active)


@router.patch("/{user_id}/leader", response_model=UserResponse, dependencies=[Depends(require_roles("ADMIN"))])
def assign_leader(user_id: UUID, data: AssignLeaderRequest, db: DBSession, current_user: CurrentUser):
...
def update_user(user_id: UUID, data: UpdateUserRequest, db: DBSession, current_user: CurrentUser):
...
def create_user(data: CreateUserRequest, db: DBSession, current_user: CurrentUser):
    return service.create_user(db, data)


@router.post("/import-excel", response_model=ImportExcelResponse, dependencies=[Depends(require_roles("ADMIN"))])
def import_excel(db: DBSession, current_user: CurrentUser, file: UploadFile = File(...)):
